...
class vecEnv:
    def __init__(self,num_agents = 1000, inp_size=2, out_size=4):
        ...
        self.agents = torch.empty((self.num_agents, 4 + self.inp_size + self.out_size), dtype=torch.float32).to(self.device)  # agent_id, [input indices], [output indices], x, y, energy
        self.e = torch.empty((0, 5), dtype=torch.float32).to(self.device)  # (agent_id, src_node, dst_node, weight, active)
        self.v = torch.empty((0, 8), dtype=torch.float32).to(self.device)  # (agent_id, node_type, state, inp_mask, out_mask, bias_mask, hidden_mask, active)

    def add_agent(self, v, e)
